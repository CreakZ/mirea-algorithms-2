...
void create_binary_file(const std::string& filename, int accs_amount) {
    ...
    std::ofstream f("data/accounts_linear.txt");
    if (!f.is_open()) {
    ...
    std::ifstream in("data/accounts_linear.txt");
    if (!in.is_open()) {
        std::cerr << "Ошибка при открытии текстового файла\n";
        return;
    }
...
int main() {
    const std::string filename = "data/accounts_linear.bin";
    ...
    auto acc = linear_search(accounts, target_number);
